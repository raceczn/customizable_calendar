...
const closeCategoryModalBtn = document.querySelector(
...
const selectedCategoryWrapper = document.querySelector(
  ".form--body__category-modal--wrapper-selection"
);
const selectedCategoryColor = document.querySelector(
  ".form--body__category-modal--wrapper__color"
);
const selectedCategoryTitle = document.querySelector(
...
export default function setEntryForm(context, store, datepickerContext) {
  ...
  function setEndDateToNextDay() {
    ...
    const nextDayTitle =
      labels.monthsShort[nextDay.getMonth()] +
      " " +
      nextDay.getDate() +
      ", " +
      nextDay.getFullYear();
  ...
  function createTimepicker(coords, currentTime, end, endLimit) {
    ...
    // Add input field for manual time entry (12-hour format)
    const manualTimeInput = document.createElement('input');
    manualTimeInput.type = 'text';
    manualTimeInput.placeholder = 'h:mm AM/PM (e.g. 2:30 PM)';
    manualTimeInput.classList.add('manual-time-input');
    timepickerTimesContainer.appendChild(manualTimeInput);
    ...
    function parse12HourTime(timeStr) {
        ...
        if (period === 'PM' && hours < 12) {
            ...
            // If no period but hours > 12, assume 24-hour format
    ...
    function validateAndSetTime(timeStr, isEnd) {
        const parsedTime = parse12HourTime(timeStr);
        if (!parsedTime) {
            manualTimeInput.classList.add('invalid-time');
            return false;
        }
        
        const { hours, minutes } = parsedTime;
        const formattedTime = `${hours}:${minutes < 10 ? '0' + minutes : minutes}`;
        const displayTime = getHour12Time(hours, minutes);
        
        if (isEnd) {
            endTimeInput.textContent = displayTime.startsWith('0') ? displayTime.slice(1) : displayTime;
            endTimeInput.setAttribute('data-form-time', formattedTime);
            
            // Validate end time is after start time if same day
            if (startDateInput.getAttribute('data-form-date') === endDateInput.getAttribute('data-form-date')) {
                const startTime = startTimeInput.getAttribute('data-form-time');
                const [startH, startM] = getHourMinInts(startTime);
            ...
            startTimeInput.textContent = displayTime.startsWith('0') ? displayTime.slice(1) : displayTime;
            startTimeInput.setAttribute('data-form-time', formattedTime);
            
            // Validate start time is before end time if same day
            if (startDateInput.getAttribute('data-form-date') === endDateInput.getAttribute('data-form-date')) {
                const endTime = endTimeInput.getAttribute('data-form-time');
                const [endH, endM] = getHourMinInts(endTime);
        ...
        manualTimeInput.classList.remove('invalid-time');
        closetimepicker();
        return true;
    }

    function setnewtime(e) {
        if (e.target === manualTimeInput) {
            if (e.key === 'Enter') {
                validateAndSetTime(e.target.value, end);
            }
            return;
        }
        ...
        const attrFor = e.target.getAttribute('data-tp-for');
    ...
    // Focus the input field when timepicker opens
